...
public abstract class ObjectWithCacheSupport implements Serializable, IObjectWithCache {

	private static final Logger logger = LoggerFactory.getLogger(ObjectWithCacheSupport.class);

	/** nazwa regionu używanego cache */
	private final String cacheRegionName;

	private final transient IObjectCache cache;
	 ...
	 */
	protected ObjectWithCacheSupport(CacheUsage cacheUsage, String cacheRegionName, CacheSharing cacheSharing,
			Boolean useDefaultRegion, Properties props) {

		/** obiekt tworzonego cache'a */
		IObjectCache preparedCache = null;
		...
		if (cacheUsage != null && !cacheUsage.equals(CacheUsage.NONE)) {
			...
			try {
				...
				if (lPprovider.isAllowed()) {
					...
					IObjectCacheProvider<?> cacheManager = CacheProviderFactory.getInstance(props);
					logger.trace("[ {} ] Zdefiniowano uzycie cache'a '{}' (impl: {}).", new Object[] {
							getClass().getSimpleName(), lPprovider.toString(), cacheManager.getClass().getName() });
					...
					if (props.get(CacheConstants.PROP_USE_DEFAULT_REGION) != null) {
						customUseDefaultRegion = Boolean
								.parseBoolean((String) props.get(CacheConstants.PROP_USE_DEFAULT_REGION));
					}
					customUseDefaultRegion = (useDefaultRegion == null ? customUseDefaultRegion : useDefaultRegion);
					if (customUseDefaultRegion) {
						logger.trace("[ {} ] Zdefiniowano uzycie domyslnego regionu '{}'.",
								new Object[] { getClass().getSimpleName(), CacheConstants.DEFAULT_REGION_NAME });
					} else {
						preparedCacheRegionName = cacheRegionName;
						logger.trace("[ {} ] Zdefiniowano uzycie regionu '{}'.",
								new Object[] { getClass().getSimpleName(), preparedCacheRegionName });
					}

					/* Utworzenie obiektu pamięci podręcznej */
					preparedCache = cacheManager.getCache(preparedCacheRegionName);
					if (customUseDefaultRegion) {
						...
						String param = (String) props
								.get(originalKey + Strings.DOTChar + CacheConstants.PROP_TIME_TO_LIVE);
						if (StringUtils.isNotBlank(param)) {
							preparedCache.setTimeToLiveSeconds(Integer.parseInt(param));
						} else {
							preparedCache.setTimeToLiveSeconds(cacheTimeToLive);
						}
					}
				...
				logger.error(String.format("[ %s ] Blad utworzenia provider'a pamieci podrecznej: %s.",
						getClass().getSimpleName(), pProvider), e);
				logger.warn("[ {} ] Definicja uzycia cache'a pominięta.", new Object[] { getClass().getSimpleName() });
				preparedCache = null;
			}
		}
	...
	public void setCacheTimeToLive(long cacheTimeToLive) {
		if (this.cacheIsAllowed()) {
			logger.debug("[{}] Ustawiam niedomyśny cacheTimeToLive: {}",
	...
	public void close() throws IOException {
		if (cacheIsAllowed() && !cacheIsShared()) {
			try {
				...
				logger.warn("Ooops... nie udalo sie niszczenie cache'a '" + this.cache.getRegionName() + "'", ignore);
			}
		}
