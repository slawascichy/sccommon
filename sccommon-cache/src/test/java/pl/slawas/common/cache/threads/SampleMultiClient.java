...
public class SampleMultiClient {
	...
	public void check(List<CachedElement> list, Properties props) throws ExecutionException {
		...
		Map<Integer, SampleCallable> threads = new HashMap<Integer, SampleCallable>();
		...
		try {
			...
			logger.debug("-->SampleMultiClient.check: packageSize: {}.", new Object[] { packageSize });
			...
			SampleCallable call = null;
			for (CachedElement lVal : list) {
				logger.trace("-->SampleMultiClient.check: dodaję '{}' z wartością: '{}'",
						new Object[] { lVal.getName(), lVal.getValue() });
				call = threads.get(threadId);
				if (call == null) {
					call = new SampleCallable(props);
			...
			int ii = 0;
			for (SampleCallable thread : threads.values()) {
			...
			logger.debug("-->SampleMultiClient.check: Przeprocesowałem: {} w czasie {} [ms]",
					new Object[] { processedDocuments, endDate - startDate });

		} catch (InterruptedException e) {
			logger.warn("-->SampleMultiClient.check: Przerwano oczekiwanie monitoringu.", e);
